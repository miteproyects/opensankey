...
with open(path, 'r') as f:
    html = f.read()
old = '<noscript>You need to enable JavaScript to run this app.</noscript>'
new = ('<noscript><div id="quartercharts-seo"><h1>QuarterCharts</h1>'
       '<p>QuarterCharts is a financial data visualization platform that turns SEC filings '
       'into interactive Sankey diagrams, quarterly income charts, and company profiles.</p>'
       '<a href="https://quartercharts.com/?page=privacy">Privacy Policy</a>'
       ' | '
       '<a href="https://quartercharts.com/?page=terms">Terms of Service</a>'
       '</div>You need to enable JavaScript to run this app.</noscript>')
if old in html:
    with open(path, 'w') as f:
        f.write(html.replace(old, new, 1))
    print('Added noscript SEO block with privacy link')
PYEOF
    fi
else
